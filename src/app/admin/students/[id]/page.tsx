...
import { formatCurrency, formatDate, calculateFeeSummary, calculateAttendancePercentage } from '@/lib/utils';
import { ArrowLeft, Save, AlertCircle, CheckCircle2, PowerOff, ShieldCheck, CreditCard, CalendarCheck } from 'lucide-react';
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800">
          ...
          <span className="text-lg font-bold text-white">{formatCurrency(student.monthly_fee)}</span>
          <p className="text-[10px] text-slate-500">Per calendar month</p>
        </div>

        <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800">
          <span className="text-[11px] text-slate-400 mb-1 block">Total Paid</span>
          <span className="text-lg font-bold text-emerald-400">{formatCurrency(feeSummary.totalPaid)}</span>
          <p className="text-[10px] text-slate-500">{payments.length} payments recorded</p>
        </div>

        <div className="p-3.5 rounded-xl bg-slate-900 border border-slate-800">
          <span className="text-[11px] text-slate-400 mb-1 block">Current Due</span>
          <span className={`text-lg font-bold ${feeSummary.currentDue > 0 ? 'text-red-400' : 'text-slate-400'}`}>
            {formatCurrency(feeSummary.currentDue)}
          </span>
          <p className="text-[10px] text-slate-500">{feeSummary.currentDue > 0 ? 'Action required' : 'Up to date'}</p>
        </div>
      </div>
